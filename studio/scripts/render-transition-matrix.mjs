...
import { transitionMatrix, transitionPhases } from '../src/transition-matrix.mjs';
import { frameStats, psnr, regionMeanLuma } from '../src/transition-quality.mjs';
...
const matrix = transitionMatrix(sourceProject);
...
const report = { version: 1, status: 'passed', thresholds: { settledPsnrDb: 42, chromeLumaRange: 8 }, entries: [], findings: [] };
try {
  for (const entry of matrix) {
    ...
    report.entries.push({
      ...
      metrics: { stats, chromeLuma: chromeLuma.map((value) => Number(value.toFixed(3))), chromeLumaRange: Number(chromeLumaRange.toFixed(3)), settledPsnrDb: displayNumber(settledPsnrDb) },
      checks,
    });
  }
} finally {
