...
const screenshot = path.join(captureDir, 'motion-registry.png');
const video = path.join(captureDir, 'motion-registry.mp4');
const sources = [fileURLToPath(import.meta.url), path.join(studioRoot, '..', 'examples', 'motion-registry', 'project.json')];

async function current() {
  try {
    const output = await stat(video);
    const inputs = await Promise.all(sources.map((file) => stat(file)));
