...
Product scenes support six composition layouts. `framed` keeps explanatory copy beside the browser, `full-bleed` makes the capture the canvas, `product-stage` presents the complete interface as a raised object, `detail-crop` gives a measured target most of the frame, `layered-product` builds depth from two views of the same capture, and `full-bleed-proof` locks a compact claim to focused evidence. Detail and proof layouts require a real `focus` rectangle. Set `presentation.caption` per beat, use `chrome` as `auto`, `show` or `hide`, and select `hero`, `statement`, `metadata`, `proof`, `label` or `silent-product` typography. Silent product beats accept no caption.

The shared motion registry resolves a stable baseline plus four specialized recipes: restrained blur/slide text, a chapter title that becomes persistent context, an evidence-linked scan and focus lock, and an outro that strips presentation layers away before the closing message. Every recipe and transition declares its renderer implementation, metadata, source fixture and deterministic fallback. They are internal version 2 presets rather than new scene types. MCP capabilities exposes the same registry catalog and each project timeline reports the selected recipe.
