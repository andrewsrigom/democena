...
`npm run benchmark:transitions` renders the production composition at the before, midpoint and settled-after phase of every registered transition and every framed/full-bleed chrome handoff. The generated `studio/output/transition-matrix/report.json` checks visible content, opaque canvas coverage, presentation-chrome visibility and stability, and a settled after frame. `contact-sheet.jpg` shows the complete matrix, while `index.html` provides an interactive A/B viewer for any two motion cases and lifecycle phases. Generated images and reports remain ignored local artifacts. Use `npm --prefix studio run matrix -- --output <absolute-directory>` to place a review bundle elsewhere.
