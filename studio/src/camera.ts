...
export function cameraAt(path: CameraStop[], time: number, viewport: { width: number; height: number }, width: number, visibleViewport?: { width: number; height: number }): Camera {
  const first = path[0];
  ...
  const a = cameraFor(before.focus, viewport, width, before.zoom ?? 1.8, visibleViewport);
  const b = cameraFor(after.focus, viewport, width, after.zoom ?? 1.8, visibleViewport);
