...
Each scene has its own duration and transition. Product beats can use a complete framed browser, full-bleed capture, raised product stage, evidence-linked detail crop, layered product view or full-bleed proof composition. Captions, presentation chrome and typographic roles are chosen per beat; `silent-product` removes authored copy while preserving the captured interface. Structured background, product and foreground layers add depth without inventing product evidence. `npm run studio:preview` exports a still of every scene.
...
`npm run benchmark:transitions` renders before, midpoint and settled-after frames for every registered transition plus automatic and explicit presentation-chrome handoffs. It blocks on blank surfaces, transparent coverage, incorrect presentation-chrome visibility, chrome flashes, drift between two production settled frames, or either settled frame differing from its time-matched transition-free destination reference. It writes a contact sheet, machine-readable report and interactive A/B viewer under the ignored `studio/output/transition-matrix/` directory.
