...
Democena creates silent product demos with real browser capture, explanatory text, and expressive motion. Read `docs/architecture.md` and `UPSTREAM.md` before changing capture or rendering.
...
- Run `npm run typecheck`, `npm test`, and `npm run build` for core changes. Run `npm run test:browser` for overlay behavior and inspect frames for visual changes.
