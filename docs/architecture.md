...
`studio/src/composition-registry.ts` defines the product composition grammar shared by Studio validation, rendering, Direction compilation and MCP discovery. Each layout declares its default caption, chrome behavior, focus requirement and background/midground/foreground responsibilities. Product layouts use only the adopted capture and measured focus geometry; decorative layers may frame that evidence but do not create product claims. Direction v2 stores per-beat composition, typography and chrome intent, then compiles those choices into the existing Project v2 scene contract.
...
The transition matrix consumes the same portable registry data from Node and Studio. It renders the production composition at before, midpoint and two settled-after frames, then checks frame content, alpha coverage, chrome luminance, explicit framed/full-bleed chrome visibility at every phase, direct stability between the production settled frames, and time-matched parity with a transition-free destination render. The generated report, contact sheet and interactive A/B viewer stay in ignored local output. A no-overlap cut advances authored entrance animations so it cannot expose an empty presentation frame while duration-based scene lifecycles keep their own clock.
