...
export function createServer(service: AgentService) {
  ...
  for (const name of Object.keys(inputs) as Operation[]) {
    ...
    server.registerTool(`democena_${name}`, { description: descriptions[name], inputSchema: inputs[name].shape, annotations: { readOnlyHint: readOnly, destructiveHint: ['save_project', 'import_media'].includes(name), idempotentHint: readOnly, openWorldHint: false } }, async (args: Record<string, unknown>): Promise<CallToolResult> => {
      try {
