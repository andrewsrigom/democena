...
4. Build canonical Direction v2 with [brief.md](references/brief.md) and [storyboard.md](references/storyboard.md). Choose its story mode and motion language, then author every beat's concept, focal action, primary subject, typographic role, density, transition intent and compatible recipe shortlist. Use [profiles.md](references/profiles.md) and read only the requested tone in [tones.md](references/tones.md). Direction v1 is read compatibility for existing plans, not an authoring target.
...
8. Render the final video only when no blocking finding remains. Inspect its artifacts, then use `democena_deliver_direction` with the final-video job ID and the exact compiled revisions. Follow [delivery.md](references/delivery.md).
